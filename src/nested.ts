...
export function toCSV(questions: Question[]): string {
    ...
    const rows = questions.map((question) => {
        const optionsCount = question.options.length; // Count the number of options
...
export function publishAll(questions: Question[]): Question[] {
    return questions.map((question) => ({
        ...question, // Spread operator to copy existing question properties
...
export function renameQuestionById(
    ...
    return questions.map((question) => {
        // Check if the current question's ID matches the target ID
        ...
        return question; // Return the original question if it doesn't match
    });
}
